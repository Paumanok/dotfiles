...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    , ((modm,               xK_d     ), spawn "dmenu_run -sb \"#500350\"")
    ...
    , ((mod1Mask           , xK_l    ), spawn "i3lock -c 000000")
    , ((modm .|. shiftMask, xK_s     ), spawn "scrot -s -e 'xclip -selection clipboard -t image/png -i $f && rm $f'")
    ]
    ++
    ...
    [((m .|. mod1Mask, k), windows $ f i)
       | (i,k) <- zip (XMonad.workspaces conf) workspace_keys
       , (f,m) <- [(W.greedyView, 0), (W.shift, shiftMask)]]
    ++
    --
